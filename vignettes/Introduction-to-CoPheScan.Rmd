...
co <-  capture.output(res.multi1 <- cophe.multitrait(cophe_multi_trait_data$summ_stat, query.snpid = query.snpid, querytrait.names = names(cophe_multi_trait_data$summ_stat), method = 'single', simplify = T, predict.hyp = T))
cophe.hier.res <- run_metrop_priors(res.multi, posterior = T, avg_posterior=T, pik=T, avg_pik = T, covar_vec = cophe_multi_trait_data$covar_vec, covar = covar, nits = 30000) 
