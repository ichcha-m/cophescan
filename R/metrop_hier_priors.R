...
run_metrop_priors <- function(
	...
	if ('hit1' %in% colnames(pp_df)) {
		pp_df$sus_labels <- rownames(pp_df) <- paste0(
			pp_df$querytrait,
			'_',
			pp_df$hit1,
			'_hit_',
	...
	if (covar) {
		if (!(length(covar_vec) == nrow(pp_df))) {
			stop(
				'Length of covar_vec should be equal to the number of traits (length(multi.dat))'
			)
	...
	if (covar & is.character(covar_vec)) {
		if (is_covar_categorical == FALSE) {
			stop(
				'covar vector of type character but is_covar_categorical set to FALSE. Note: only 2 categories supported'
			)
	...
	if (!covar & !is.null(covar_vec)) {
		message(
			'covar_vec supplied but covar set to false, setting covar_vec to 1. Set covar to True if covariate to be included'
		)
	...
	if (avg_posterior) {
		avg.posterior <- average_posterior_prob(
			res.metrop$parameters,
			lbf_mat,
			nsnps = nsnps,
			covar_vec = covar_vec,
	...
	# res.metrop$data = pp_df %>%
	# 	dplyr::select_if(!grepl('PP.Hn|PP.Ha|PP.Hc', colnames(pp_df)))
	res.metrop$data = pp_df %>%
		dplyr::select(-dplyr::matches('PP.Hn|PP.Ha|PP.Hc'))
