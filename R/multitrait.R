	...
	for (idx in seq_along(trait.dat)) {
		...
		if (qv %in% dat$snp) {
			if (method == 'susie') {
				...
				cophe_results[[idx]] <- cophe.single(
					dat,
					querysnpid = qv,
					querytrait = qt,
					...
...
#'
#' @return  dataframe with posterior probabilities of Hn, Hc and Ha
multitrait.simplify <- function(multi.dat, only_BF = FALSE) {
	...
	for (trait in seq_along(multi.dat)) {
		...
		if (any(names(dat$summary) %in% 'hit1')) {
			pp$sus_labels <- rownames(pp) <- paste0(
				names(multi.dat)[trait],
				'_hit_',
				dat$summary$hit2
			)
		} else {
