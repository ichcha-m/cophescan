...
cophe_plot <- function(multi.dat, causal.snpid, thresh_Hc=0.5, thresh_Ha=0.5, traits.dat=NULL){
  ...
  if (!is.null(traits.dat)){
    ...
    g3 <- ggplot(aes(x=x, y=y, label=L1), data=df_p) +
      geom_point(col='maroon',alpha=0.8, aes(shape=beta), size=5) +
...
plot_cophe_ternary <- function(multi.dat, traits.dat=NULL, plot_pval=F, thresh_Hc=0.5, thresh_Ha=0.5){
  pp_df <- prepare_plot_data(multi.dat, thresh_Hc, thresh_Ha, cophe.plot = T, hmp=F)
    ...
    trn = trn+geom_point(aes(color=pval_plot, shape=beta_plot),fill='black', size=6)+ theme_linedraw() + scale_color_viridis_c(alpha = 0.8, n.breaks = 6) + theme_arrowsmall()+ labs(color="-log10(pval)",shape="beta", size=10) + Tarrowlab("")+ Larrowlab("")+ Rarrowlab("") +  theme_nomask()+  scale_shape_manual(values= c("\u25BC", "\u25B2"))
  } else {
    trn = ggtern(data=pp_df, aes_string(x="Hn",y="Hc",z="Ha"))
    trn = trn+geom_point(aes(color=Hc),fill='black',  size=6)+ theme_linedraw() + scale_color_viridis_c(alpha = 0.8, n.breaks = 6, option = "plasma") + theme_arrowsmall()+ labs(color="Hc", size=10)  + Tarrowlab("")+ Larrowlab("")+ Rarrowlab("")+  theme_nomask()
  }
