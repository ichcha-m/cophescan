...
get_beta <- function(traits.dat, querysnpid, querytrait) {
	pval_plot <- sapply(traits.dat, function(d) {
		-(pnorm(
			-abs(d$beta[querysnpid]) / sqrt(d$varbeta[querysnpid]),
			log.p = TRUE
		) +
			log(2)) /
			log(10)
...
#' @param beta_p data.frame (from the `get.beta` function) with four columns : 1. "beta_plot": indicating beta direction (p or n) 2. "pval_plot": -log10(pval) of the queried variant 3. "querysnp" 4. "querytrait".
...
cophe_plot <- function(
	...
	if (length(querysnpid) != 1) {
		stop(
			"length(querysnpid)!=1, Pass traits corresponding only to a single queryvariant"
		)
	}
	if (is.null(traits.dat) & is.null(beta_p)) {
		warning(
			'Trait summary stat data or beta_p data.frame required for pval PheWAS plot'
		)
	}
	if (
		!is.null(beta_p) &
			!any(
				colnames(beta_p) %in%
					c("beta_plot", "pval_plot", "querysnpid", "querytrait")
			)
	) {
		stop(
			"Check the colnames of the beta_p data.frame or pass NULL if pval PheWAS plot not required"
		)
	}

	plot_list <- list()
	pp_df <- prepare_plot_data(
		multi.dat,
		querysnpid = querysnpid,
		query_trait_names = query_trait_names,
		thresh_Hc = thresh_Hc,
		thresh_Ha = thresh_Ha,
		cophe.plot = TRUE,
		hmp = FALSE
	...
	L2 <- pp_df$L2
	g1 <- suppressWarnings(
		ggplot(aes(x = x, y = PP.Hc, label = L1), data = pp_df) +
			...
			theme(
				...
				axis.line = element_line(color = 'grey22')
			) +
	...
	g2 <- suppressWarnings(
		ggplot(aes(x = x, y = PP.Ha, label = L2), data = pp_df) +
			geom_point(
				col = 'forestgreen',
				alpha = 0.8,
				size = 5,
	...
	if ((!is.null(traits.dat)) | (!is.null(beta_p))) {
		...
		df_p <- data.frame(
			x = seq_len(nrow(beta_p)),
			y = pval_plot,
			label = L1,
			beta = as.factor(as.character(beta_plot))
		)
...
#' @return heatmap of posterior probabilities of the phenotypes above the set threshold
#' @export
#'
cophe_heatmap <- function(
	multi.dat,
	querysnpid,
	query_trait_names,
	thresh_Hc = 0.5,
